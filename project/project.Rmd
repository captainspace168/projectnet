...
```
diffrent ratings 
the table below depicts the diffrent age ratings and what they mean
```{r}
...
```
this chart depicts all of the diffrent shows streaming and what there age rating is 
```{r}
netflixShows %>% 
select(title,rating ) %>% 
group_by(rating ) %>% 
...
```
this chart shows the diffrent raating description
```{r}
netflixShows %>% 
...
netflixShows %>% 
  ...
  summarise(averge_rating_per_year =mean(user_rating_score ,na.rm=TRUE))
```
...
datasets cited 
thank you  professor Rei Sanchez-Arias for provideing the netflix dataset
